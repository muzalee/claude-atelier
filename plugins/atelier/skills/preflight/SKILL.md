...
**Report inline. Do not write a report file.** What has to outlive the conversation already does: step 5 edits the plan itself, and a `preflight: BLOCKED — <PF-n>` marker sits on every step that could not be resolved. `/build` reads those markers. A separate report beside the plan is a second document saying the plan is wrong, and it goes stale the moment someone fixes a step.
